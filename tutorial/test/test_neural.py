...
from tutorial.network import _URL, path_to_zip, PATH, train_dir, validation_dir, Network
...
def test_data_preprocessing_ValidationBatches():
    assert (tf.data.experimental.cardinality(nn.validation_dataset) == 32) and (
            tf.data.experimental.cardinality(nn.test_dataset) == 6)
